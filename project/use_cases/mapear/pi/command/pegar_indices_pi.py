...
class PegarIndicesPi(PegarIndicesInterface):
    ...
    def execute(self, plano_interno_df: pd.DataFrame) -> dict:
        ...
        for i in range(len(list_indices_planos_internos)):
            ...
            for j in range(
                list_indices_planos_internos_verification[i],
                list_indices_planos_internos_verification[i + 1] + 1,
            ):
                aux_counter = 0
                ...
                if (
                    re.search(
                        pattern_elemento_despesa_nome_tipo_1, str(plano_interno_df[1][j])
                    )
                    and re.search(
                        pattern_elemento_despesa_codigo_tipo_1,
                        str(plano_interno_df[2][j]),
                    )
                ) or (
                    re.search(
                        pattern_elemento_despesa_nome_e_codigo_tipo_2,
                    ...
                    dict_planos_internos[list_indices_planos_internos[i]].append(
                        dict_elemento_despesa
                    )
