    ...
    def execute(self, dict_indices: dict, plano_interno: pd.DataFrame) -> dict:
        ...
        for w in dict_indices:
            ...
            if not re.match(r'-\d', plano_interno[1][w]):
                ...
                dotacao_plano_interno_siafi_dasf = self.utils.value_hygienization(plano_interno[3][list(dict_indices[w][0].keys())[0]])
    
                dict_resultado_plano_interno_siafi_dasf[nome_plano_interno_siafi_dasf] = {'indice plano interno': w,'indice total': list(dict_indices[w][0].keys())[0] ,'valor': dotacao_plano_interno_siafi_dasf, 'elementos de despesa': {}}
                ...
                for z in dict_indices[w]:
                    ...
                    for y in z:
                        ...
                        for x in range(len(z[y])): 
                            ...
                            nome_elemento_despesa_siafi_dasf = plano_interno[2][dict_indices[w][0][y][x]]
                            nome_elemento_despesa_siafi_dasf = nome_elemento_despesa_siafi_dasf[:1] + '.' + nome_elemento_despesa_siafi_dasf[1:2] + '.' + nome_elemento_despesa_siafi_dasf[2:4] + '.' + nome_elemento_despesa_siafi_dasf[4:]
                            dotacao_elemento_despesa_siafi_dasf = self.utils.value_hygienization(plano_interno[3][dict_indices[w][0][y][x]])
                    
                            dict_resultado_plano_interno_siafi_dasf[nome_plano_interno_siafi_dasf]['elementos de despesa'][nome_elemento_despesa_siafi_dasf] = {'indice': z[y][x], 'valor': dotacao_elemento_despesa_siafi_dasf}
