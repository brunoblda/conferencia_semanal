...
class PegarIndicesPiSeof(PegarIndicesInterface):
    ...
    def execute(self, plano_interno_df: pd.DataFrame) -> dict:
        ...
        indices_planos_internos_seof = plano_interno_df[matched_rows_planos_internos_seof].index
        ...
        list_indices_planos_internos_verification_seof =list_indices_planos_internos_seof.copy()

        # adicionando o ultimo indice do dataframe para a lista de verificacao
        list_indices_planos_internos_verification_seof.append(len(plano_interno_df.index)-1)
        ...
        for i in range(len(list_indices_planos_internos_seof)):
            ...
            for j in range(list_indices_planos_internos_verification_seof[i],list_indices_planos_internos_verification_seof[i+1]+1):
                aux_counter = 0
        
                # Verifica se os patterns de elemento de despesa acontecem
                if re.search(pattern_elemento_de_despesa_seof, str(plano_interno_df[1][j])):
                    ...
                    dict_planos_internos_seof[list_indices_planos_internos_seof[i]].append(dict_elemento_despesa)
                ...
                if re.search(pattern_desdobramento_elemento_despesa_seof, str(plano_interno_df[1][j])):
            
                    # adiciona na lista value do elemento de despeas, o desdobramento de despesa
                    dict_planos_internos_seof[list_indices_planos_internos_seof[i]][aux_counter-1][indice_elemento_despesa].append(j)
