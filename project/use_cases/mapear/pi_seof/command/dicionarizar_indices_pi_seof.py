...
class DicionarizarIndicesPiSeof(DicionarizarIndicesInterface):
    ...
    def execute(self, dict_indices: dict, plano_interno: pd.DataFrame) -> dict:
        ...
        for w in dict_indices:
            ...
            nome_plano_interno_seof = re.search(
                pattern_plano_interno_seof, plano_interno[1][w]
            ).group()

            dotacao_plano_interno_seof = self.utils.value_hygienization(
                plano_interno[2][w]
            )

            dict_resultado_plano_interno_seof[nome_plano_interno_seof] = {
                "indice": w,
                "valor": dotacao_plano_interno_seof,
            ...
            for z in dict_indices[w]:
                ...
                for y in z:
                    ...
                    nome_elemento_despesa_seof = (
                        nome_elemento_despesa_seof[:1]
                        + "."
                        + nome_elemento_despesa_seof[1:2]
                        + "."
                        + nome_elemento_despesa_seof[2:4]
                        + "."
                        + nome_elemento_despesa_seof[4:]
                    )

                    dotacao_elemento_despesa_seof = self.utils.value_hygienization(
                        plano_interno[2][y]
                    )
                    ...
                    dict_resultado_plano_interno_seof[nome_plano_interno_seof][
                        ...
                        "indice": y,
                        "valor": dotacao_elemento_despesa_seof,
                    ...
                    for x in z[y]:
                        ...
                        nome_desdobramento_despesa_seof = re.search(
                            pattern_desdobramento_elemento_despesa_seof,
                            plano_interno[1][x],
                        )[0]

                        dotacao_desdobramento_despesa_seof = (
                            self.utils.value_hygienization(plano_interno[2][x])
                        )
                        ...
                        if (
                            nome_desdobramento_despesa_seof
                            not in dict_resultado_plano_interno_seof[
                                nome_plano_interno_seof
                            ]["elementos de despesa"][nome_elemento_despesa_seof][
                                "desdobramentos de despesa"
                            ]
                        ):
                            dict_resultado_plano_interno_seof[nome_plano_interno_seof][
                                "elementos de despesa"
                            ][nome_elemento_despesa_seof]["desdobramentos de despesa"][
                                nome_desdobramento_despesa_seof
                            ] = {
                                "indices": [x],
                                "valor": dotacao_desdobramento_despesa_seof,
                            }
                        else:
                            dict_resultado_plano_interno_seof[nome_plano_interno_seof][
                                "elementos de despesa"
