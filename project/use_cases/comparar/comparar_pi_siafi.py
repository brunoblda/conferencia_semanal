        ...
        for n in pi:
            ...
            if n in pi_siafi:
                if pi[n]['valor'] != pi_siafi[n]['valor']:
                    response += (f"|{n:^15}|{'':^17}|" + self.utils.trocar_virgulas_e_pontos(f"{pi[n]['valor']:^15,.2f}|{pi_siafi[n]['valor']:^15,.2f}|{pi[n]['valor'] - pi_siafi[n]['valor']:^17,.2f}|\n"))
                ...
                for m in pi[n]['elementos de despesa']:
                    ...
                    if m in pi_siafi[n]['elementos de despesa']:
                        if pi[n]['elementos de despesa'][m]['valor'] != pi_siafi[n]['elementos de despesa'][m]['valor']:
                            response += (f"|{n:^15}|{m:^17}|" + self.utils.trocar_virgulas_e_pontos(f"{pi[n]['elementos de despesa'][m]['valor']:^15,.2f}|{pi_siafi[n]['elementos de despesa'][m]['valor']:^15,.2f}|{pi[n]['elementos de despesa'][m]['valor'] - pi_siafi[n]['elementos de despesa'][m]['valor']:^17,.2f}|\n"))

                    # se o elemento de despesa não estiver no dicionario de elementos de despesa do plano interno do siafi
                    else:
                        response += (f"|{n:^15}|{m:^17}|" + self.utils.trocar_virgulas_e_pontos(f"{pi[n]['elementos de despesa'][m]['valor']:^15,.2f}|{'':^15}|{'Não encontrado':^17}|\n"))

            # se o plano interno não estiver no dicionario de planos internos do siafi
            else:
                response += (f"|{n:^15}|{'':^17}|" + self.utils.trocar_virgulas_e_pontos(f"{pi[n]['valor']:^15,.2f}|{'':^15}|{'Não encontrado':^17}|\n"))
