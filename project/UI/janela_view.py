...
class JanelaView(ctk.CTk):
    ...
    def __radio_button_seof_siafi(self):
        ...
        self.radio_button_pi_seof = ctk.CTkRadioButton(self, text="Comparar PI X SEOF", font=('default', 20) ,variable=self.radio_var,command=self.__on_radio_button, value=1)
        self.radio_button_pi_seof.grid(row=1, column=0, padx=(70,0),pady=20, sticky='w')
        self.radio_button_pi_siafi = ctk.CTkRadioButton(self, text="Comparar PI X SIAFI", font=('default', 20), variable=self.radio_var, command=self.__on_radio_button, value=2)
        self.radio_button_pi_siafi.grid(row=1, column=1, padx=(0,70),pady=20, sticky='e')
        
    
    def __label_plano_interno(self):
        self.plano_interno_label_text = ctk.CTkLabel(self, text="Caminho do Arquivo do Plano Interno:", font=('default', 20))
        ...
        self.plano_interno_select_button = ctk.CTkButton(self, text="Selecionar Arquivo", font=('default', 20), command=self.select_file_plano_interno)
        self.plano_interno_select_button.grid(row=4, column=0, pady=5, columnspan=2)

        
    def __label_seof(self):
        self.seof_label_text = ctk.CTkLabel(self, text="Caminho do Arquivo do SEOF:", font=('default', 20))
        ...
        self.seof_button = ctk.CTkButton(self, text="Selecionar Arquivo", font=('default', 20), command=self.select_file_seof)
    ...
    def __label_siafi(self):
        self.siafi_label_text = ctk.CTkLabel(self, text="Caminho do Arquivo do SIAFI:", font=('default', 20))
        ...
        self.siafi_button = ctk.CTkButton(self, text="Selecionar Arquivo", font=('default', 20), command=self.select_file_siafi)
    ...
    def __button_comparar(self):
        self.comparar_button = ctk.CTkButton(self, text="Comparar", font=('default', 20), command=self.on_compare)
        self.comparar_button.grid(row=18, column=0, pady=(20,0), columnspan=2)

    def __response_message(self):
        self.response_message = ctk.CTkLabel(self, text="", font=('default', 20), text_color='red')
    ...
    def on_compare(self):
        if self.radio_var.get() == 0:
            self.response_message.configure(text="Por favor, selecione o tipo de comparação") 
        else:
            self.__controller.criar_pastas()
            self.open_pop_up_loading("Comparador de PIs","Carregando, por favor aguarde...")
            if self.radio_var.get() == 1:
